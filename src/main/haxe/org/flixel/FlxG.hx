...
class FlxG 
{
	...
	static public function playMusic(Music:Dynamic, ?Volume:Float = 1.0):Void
	{
		...
		if (_soundCache.exists(Music)) {
			_soundCache.get(Music).play(0, -1, _soundTransform);
		}
		else {
			sound = Assets.getSound(Music);
			
			_soundCache.set(Music, sound);
			sound.play(0, -1, _soundTransform);
		}
        #else*/
		if (music == null)
		{
